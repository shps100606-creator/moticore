...
- **目前角色**：PM
- **目前階段**：execution
- **目前步驟**：Step 6 — Worker 執行中
- **下一步**：Worker 完成後 PM 驗收
- **禁止前進條件**：WN1 / WN2 任一 blocked 未處理前不進入 Release
- **WN 文件**：WN1.md（後端：迴圈偵測 + Analytics + Discussions fetch）、WN2.md（前端 + 身份）
- **版本摘要**：（封版時填）
...
```
⚠️ **VERCEL_TOKEN 尚未加入 heartbeat.yml**（M3 的前提，WN1-3 執行後解決）
...
| VERCEL_TOKEN 需確認 | heartbeat.yml 目前沒有此變數，WN1-3 執行後需手動到 repo settings 確認 secret |
| Giscus 需 GitHub Discussions 啟用 | 需在 repo settings 啟用（目前未確認） |
| moti 無法感知讀者 | Giscus 上線後需 WN1-4 持續抓取 Discussions |
...
| GitHub Discussions 未啟用 | 中 | 中 | WN1-4 有 graceful degradation |
...
| 批次 | 內容 | 產出 | 狀態 |
|------|------|------|------|
| B1 後端 | M1 迴圈偵測 + M3 Analytics + M2 Discussions fetch（WN1） | preprocessor.py、run.py、issues.py、heartbeat.yml | planning |
| B2 前端 + 身份 | M2 Giscus 元件 + MOTIVE.md（WN2） | GiscusComments.vue、slug.vue、MOTIVE.md | planning |
...
| WN1-2 | PJ | CR | M3：preprocessor `_fetch_analytics()` + Layer 2 注入 + run.py 傳參 | `WN1.md#WN1-2` | open | |
| WN1-3 | PJ | CR | M3：heartbeat.yml 加入 VERCEL_TOKEN + VERCEL_PROJECT_ID | `WN1.md#WN1-3` | open | |
| WN1-4 | PJ | PR | M2：issues.py `fetch_discussions()` + run.py 寫入 memory/giscus-comments.md | `WN1.md#WN1-4` | open | |
| WN2-1 | PJ | PR | M2：建立 `GiscusComments.vue` 元件 + 嵌入文章頁 | `WN2.md#WN2-1` | open | |
| WN2-2 | PJ | CR | M2：MOTIVE.md 加入 Giscus 感知說明（擁有者可用 §READ_REQUEST 讀 memory/giscus-comments.md） | `WN2.md#WN2-2` | open | |
...
|----------|------|----------|----------|----------|-----------|
| `VP.md` | `_docs/versions/v0.4.0/VP.md` | 新建 | ✅ | — | |
| `WN1.md` | `_docs/versions/v0.4.0/WN1.md` | 新建 | ✅ | — | |
| `WN2.md` | `_docs/versions/v0.4.0/WN2.md` | 新建 | ✅ | — | |
| `preprocessor.py` | `agent/preprocessor.py` | 更新 | | | M1 + M3 |
| `run.py` | `agent/run.py` | 更新 | | | M3 傳參 + M2 Discussions 寫入 |
| `issues.py` | `agent/issues.py` | 更新 | | | M2 `fetch_discussions()` |
| `heartbeat.yml` | `.github/workflows/heartbeat.yml` | 更新 | | | M3 環境變數 |
| `GiscusComments.vue` | `web/components/GiscusComments.vue` | 新建 | | | M2 前端 |
| `[...slug].vue` | `web/pages/posts/[...slug].vue` | 更新 | | | 嵌入 Giscus |
| `MOTIVE.md` | `core/MOTIVE.md` | 更新 | | | M2 感知說明 |
| `memory/giscus-comments.md` | `memory/giscus-comments.md` | 程式寫入 | | | 由 run.py 寫入，有留言時才存在 |
...
| M1 迴圈偵測邏輯驗證 | 未測 | |
| M2 Giscus 前端顯示 | 未測 | |
| M2 Discussions fetch | 未測 | |
| M3 Analytics 注入 | 未測 | |
| CI / GitHub Actions heartbeat | 未測 | |
| Vercel 部署 | 未測 | |

**已知問題**：
- VERCEL_TOKEN 需在 repo Settings → Secrets → Actions 確認或新增
- VERCEL_PROJECT_ID 需在 Vercel 儀表板取得
- GitHub Discussions 需確認已在 repo Settings 啟用
- Giscus data-repo-id / data-category-id 需到 giscus.app 取得
