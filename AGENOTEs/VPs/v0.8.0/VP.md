...
| 任務 | 完成標準 | 驗證方式 | 狀態 |
|------|---------|---------|------|
| D1 重複貼文 bug 修復 | 同一 date+window 至多一個 `web/content/posts/` 檔案 | `py_compile`；程式碼審查確認 slug 生成邏輯 | ✅ 完成，待下次真實發文時段觀察確認 |
| D2 Giscus 回覆機制 | moti 能看到讀者留言、能對指定留言發出串接回覆、已回覆留言不重複提示 | `py_compile`；mock GraphQL 呼叫的 smoke test（`parse_remarks` 解析、`handle_giscus_replies` 派發與去重、`fetch_discussions` label_map 產生）三項皆通過 | ✅ 程式碼完成並通過模擬測試，**真實 API 呼叫未驗證**（見技術決策 D2「未解決問題」）|

**尚未驗證的風險**：D2 的 GraphQL mutation 從未打過真實 GitHub API，`GITHUB_TOKEN` 的 `discussions: write` 權限是否足夠、Giscus 的 discussion↔留言 mapping 是否如預期，都要等下一次 CI 心跳實際執行後才能確認。

**部署狀態（260709 補充）**：WN1 原本只在 feature branch `claude/agenote-enablement-izknyf`，heartbeat.yml 對 `main` 執行，直到合併前兩次真實心跳（11:30、11:51 UTC）用的都還是舊程式碼。已開 PR #47 並合併進 `main`（merge commit `b3e898d`），確認 `main` 上 `handle_giscus_replies` 存在、`_slugify` 已移除、`discussions: write` 權限已生效。下一次心跳（main 排程每 30 分鐘一次）起才會真正跑到新程式碼。
